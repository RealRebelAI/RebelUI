r"""Restore original tensor shapes in a GGUF via comfy.gguf.orig_shape KVs.

ggml trims trailing singleton dimensions when a file is rewritten (llama-quantize
does this), so a [1, 4096] tensor becomes [4096] and ComfyUI reports:

    size mismatch for keyframes_abs_pos_embedding: copying a param with shape
    torch.Size([4096]) ... current model is torch.Size([1, 4096])

ComfyUI-GGUF reads comfy.gguf.orig_shape.<tensor> and restores the shape.
This compares a GGUF against its source safetensors and adds those KVs for
every tensor whose shape was altered.

Usage:
  python_embeded\python.exe gguf_fix_shapes.py <file.gguf> --from <src.safetensors>
  python_embeded\python.exe gguf_fix_shapes.py <file.gguf> --from <src> --dry-run

Header-only where it fits; otherwise shifts data in place (no temp file).

--- original tool docs follow ---
Write a model config blob into a GGUF's KV metadata.
...
def main():
    ...
    if not path.exists():
        print(f"[shapes] not found: {path}"); sys.exit(1)
    if src is None or not src.exists():
        print("[shapes] need --from <src.safetensors>"); sys.exit(1)
    ...
    with open(src, "rb") as f:
        ...
        sh = json.loads(f.read(n).decode("utf-8"))
    sh.pop("__metadata__", None)
    src_shapes = {}
    for k, v in sh.items():
        base = k[len("model.diffusion_model."):] if k.startswith(
            "model.diffusion_model.") else k
        src_shapes[base] = list(v["shape"])
        src_shapes[k] = list(v["shape"])
    ...
    for t in reader.tensors:
        want = src_shapes.get(t.name)
        if want is None:
            continue
        have = list(reversed([int(d) for d in t.shape.tolist()]))
        if have == want:
            continue
        n_have = 1
        for d in have:
        ...
        for d in want:
            n_want *= d
        if n_have != n_want:
            continue          # genuine difference, not a trimmed dim
        key = f"comfy.gguf.orig_shape.{t.name}"
        if key in fields:
            continue
        needed[key] = want
        print(f"[shapes] {t.name}: {have} -> {want}")

    print(f"[shapes] {len(needed)} shape(s) to restore")
    if not needed:
        print("[shapes] nothing to do")
        return
    if dry:
        print("[shapes] dry run; nothing written")
        return
    ...
    for key, shape in needed.items():
        w_str(parts, key)
        parts.append(struct.pack("<I", V_ARRAY))
        parts.append(struct.pack("<I", 5))          # INT32
        parts.append(struct.pack("<Q", len(shape)))
        for d in shape:
            parts.append(struct.pack("<i", int(d)))
    ...
    if new_base == old_base:
        ...
        print(f"[shapes] in-place header write; data untouched")
    else:
        ...
        print(f"[shapes] header grew {delta} B -> shifting {data_len/1e9:.2f} "
        ...
        with open(path, "r+b") as f:
            ...
            while pos > 0:
                ...
                if done % (CHUNK * 8) == 0:
                    print(f"[shapes]   shifted {done/1e9:.1f}/"
            ...
            f.flush()
            os.fsync(f.fileno())
        print(f"[shapes] wrote new header ({new_base} B)")

    r2 = GGUFReader(str(path))
    missing = [k for k in needed if k not in r2.fields]
    still_cfg = "config" in r2.fields
    counts = {}
    for t in r2.tensors:
        counts[Q(t.tensor_type).name] = counts.get(Q(t.tensor_type).name, 0) + 1
    print(f"[shapes] VERIFY: {len(r2.tensors)} tensors, qtypes {counts}")
    print(f"[shapes] VERIFY: {len(needed) - len(missing)}/{len(needed)} "
          f"orig_shape KVs present; config KV preserved: {still_cfg}")
