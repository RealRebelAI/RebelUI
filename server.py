...
# Windows embedded Python can run in isolated mode and omit the script folder
# from sys.path. Add RebelUI's own directory explicitly before importing local
# modules such as quantizer.py.
HERE = Path(__file__).resolve().parent
if str(HERE) not in sys.path:
    sys.path.insert(0, str(HERE))

import quantizer as rebel_quant
...
def make_app(runner, folder_paths, comfy, out_dir):
    ...
    async def quant_resolve(request):
        try:
            body = await request.json()
            value = str(body.get("source", "")).strip()
            if not value:
                return web.json_response({"error":"Enter a HuggingFace repository or local path."}, status=400)
            plan = await asyncio.get_running_loop().run_in_executor(None, rebel_quant.resolve_plan, value)
            return web.json_response(plan)
        except PermissionError as e:
            return web.json_response({"error":str(e), "gated":True}, status=403)
        except Exception as e:
            import traceback
            return web.json_response({"error":str(e), "detail":traceback.format_exc()}, status=400)

    async def quant_run(request):
        body = await request.json()
        resp = web.StreamResponse(headers={"content-type":"application/x-ndjson"})
        await resp.prepare(request)
        loop = asyncio.get_running_loop()
        ...
        def emit(msg, level=None):
            send({"log":str(msg), "level":level, "quant":True})

        try:
            source = str(body.get("source","")).strip()
            plan = await loop.run_in_executor(None, rebel_quant.resolve_plan, source)
            if not body.get("confirmed"):
                raise RuntimeError("Plan confirmation is required before downloading weights.")
            result = await loop.run_in_executor(
                None, lambda: rebel_quant.run_quantization(plan, body, emit)
            )
            await resp.write((json.dumps({"quant_done":result})+"\n").encode())
            await resp.drain()
        except Exception:
            import traceback
            await resp.write((json.dumps({"log":traceback.format_exc(),"level":"er","quant":True})+"\n").encode())
        await resp.write_eof()
        return resp
    ...
    app.router.add_post("/api/quant/resolve", quant_resolve)
    app.router.add_post("/api/quant/run", quant_run)
